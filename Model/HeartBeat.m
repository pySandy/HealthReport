...
#pragma mark- 外部调用的方法
- (void)startHeartRatePoint:(void (^)(NSDictionary *))backPoint Frequency:(void (^)(NSInteger))frequency Error:(void (^)(NSError *))error
{
...
- (void)setupCapture
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied)
    {
        ...
        NSError *err = [NSError errorWithDomain:error code:100 userInfo:@{@"content":@"相机不可用，或没有相机权限"}];
        if (self.backPoint)
            self.Error(err);
            ...
            AVCaptureConnection *connection = [self.output connectionWithMediaType:AVMediaTypeAudio];
...
//captureOutput - >当前output sampleBuffer->样本缓冲 connect-> 捕获连接
